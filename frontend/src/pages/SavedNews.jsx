...
function SavedNews() {
  ...
  const savedIndex = useMemo(() => {
    return index.filter(item => bookmarks.includes(item.date))
  }, [index, bookmarks])

  const trie = useMemo(() => {
    const t = new Trie()
    savedIndex.forEach((item, i) => {
  ...
  const filteredNews = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      if (words.length > 0) {
        ...
        finalResults = (commonIndices || []).map(idx => savedIndex[idx]);
      }
    }
  ...
  }, [searchQuery, searchDate, sortOrder, savedIndex, trie]);
      ...
      <main className="archive-layout">
        ...
        <div className="news-list">
          {currentNews.length > 0 ? (
            ...
            <p className="no-results">No saved news found matching your criteria</p>
          )}
        </div>
